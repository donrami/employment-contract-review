...
### Phase 5 — Report
1. Emit THREE artifacts — JSON (schema § 3), markdown report (template § 4), and a self-contained HTML view. Same content, three views: JSON is canonical; markdown is the source-of-truth human view; HTML is a derived rendering of the markdown and MUST be produced with `python3 tools/render_report.py <report.md> <report.html>` (severity badges, collapsible annex, print button — no manual HTML authoring, no content re-wording).
